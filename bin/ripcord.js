...
function prog() {
    try {
        ...
        /* istanbul ignore next */
        if (__filename !== localRipcordFilename)
    ...
    program
        ...
        .option('--concurrency <int>', 'number of concurrent copy to requests permitted during sync. defaults to 8', v => parseInt(v, 10))
    ...
    Promise.resolve(cmdP)
        ...
        if (action.name === 'report')
            post_process_report_1.default(action, result);
    });
    return program;
}
module.exports = prog();
