...
ava('license check, web-build', async t => {
  ...
  const transformStub = sinon.stub(uiBuild, 'applyWebBuildTransform')
  .callsFake((pkgs, opts) => {
    pkgs.webpack = wpStub
...
ava('license dump, web-build', async t => {
  ...
  const wStub = uiBuild.applyWebBuildTransform
  const stub = sinon.stub(uiBuild, 'applyWebBuildTransform')
  .callsFake((pkgs, opts) => {
    pkgs.webpack = wpStub
    return wStub.call(uiBuild, pkgs, opts)
  })
  ...
  let goldenDump = await fs.readFile(goldenFilename)
  goldenDump = goldenDump.toString().trim()
