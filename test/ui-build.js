...
const { dummyUiBuildProjectDirname, webpackConfigFilename, wpStub } = common;
...
tape('ui build fails without build config', t => {
    ...
    Promise.resolve()
        ...
        uiBuild.applyWebBuildTransform(pkgs);
    })
        .catch(err => {
...
tape('ui dependencies transform flatly', t => {
    ...
    Promise.resolve()
        ...
        return uiBuild.applyWebBuildTransform(pkgs, { uiBuild: true, webpackConfig: webpackConfigFilename });
    })
        .then(pkgs => {
        t.equal(pkgs['dummy-pkg;0.0.1'].production, true, 'devDep consumed by build transfers to prod dep');
        t.notOk(Object.keys(pkgs).some(name => !!name.match('unused-pkg')), 'unused dependency (not dev dep!) omitted from report');
    })
