...
module.exports = {
  ...
  _checkLicenses (pkgs, opts, ripcord) {
    ...
    const flatPkgs = flattenPkgs({ pkgs, flatSet: {}, root: true })
    for (let name in flatPkgs) {
      /* istanbul ignore next */
      if (this._hasWhitelistedLicense(flatPkgs[name])) delete flatPkgs[name]
    }
   ...
   * @returns {IPkgSet}
   */
  _mergeLicensesToPkgSet (lPkgs : any, pkgs : IPkgSet) : IPkgSet {
  ...
  _reportToCSV (pkgs) {
    for (let name in pkgs) {
      ...
      for (let propName in pkg) {
        const prop = pkg[propName]
        if (pkg.hasOwnProperty(propName)) {
          if (Array.isArray(prop)) pkg[propName] = prop.join(';')
        }
      }
    }
