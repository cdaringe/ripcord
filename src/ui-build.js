...
const { uniqBy, values, keyBy, set } = require('lodash');
...
module.exports = {
    ...
    applyWebBuildTransform(pkgs, opts) {
        ...
        return this.getWebpackDeps(opts)
            ...
            for (let wpKey in wpDepsByKey) {
                if (flatPkgsByKey[wpKey])
                    flatPkgsByKey[wpKey].production = true;
            }
     ...
     * @returns {object}
     */
    _getBabelConfig(configFilename) {
        let config;
    ...
    _getWebpackedNodeModules({ webpackDir, webpackConfig }) {
        const wpPath = path.join(counsel.targetProjectRoot, 'node_modules', 'webpack');
        const wp = require(wpPath); // naughty daddy...
        ...
        return pify(wp)(wpJsonProfileConfig)
            .then(stats => this._wpStatsToPkgMeta(stats, webpackDir))
    ...
    _getWebpackedNodeModulesFilenames({ modules, webpackDir }) {
        ...
        return modules
            ...
            while (relRoot.indexOf(wpSep) >= 0)
                relRoot = relRoot.replace(wpSep, wpNm);
            return relRoot;
        })
    ...
    _wpPkgMetaToBundleSet(pkgReqs) {
        ...
        return uniqBy(bundledNodeModules, pkg => pkg_1.key(pkg));
    },
    /**
     * @private
     * @param {any} stats
     * @returns {Promise}
     */
    _wpStatsToPkgMeta(stats, webpackDir) {
        const modules = stats.toJson().modules;
        const packedNodeModuleFilenames = this._getWebpackedNodeModulesFilenames({ modules, webpackDir });
        return this._getUpstreamPackageJsons(packedNodeModuleFilenames);
    },
    /**
     * @deprecated
    ...
    _extractWebpackNodeModulePackageName(namePath) {
        ...
        return (function () {
            ...
            if (!namePath.charAt(pkgNameStartNdx))
                throw new Error('unable to find pkg name');
