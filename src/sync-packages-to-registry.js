...
module.exports = {
   ...
   * @param {object} opts
   * @param {boolean} opts.dryRun
   * @param {boolean} opts.forceCopy copy from /cache-repo even if not present in /src repo. used for testing
   * @returns Promise
   */
  _syncPackage (pkg, opts) {
    ...
    if (pkg.action === ACTION_SKIP && !opts.forceCopy) return Promise.resolve()
    /* istanbul ignore next */
