...
module.exports = {
  ...
  applyWebBuildTransform (pkgs : IPkgSet, opts : any) {
    ...
    .then(wpDeps => {
      ...
      const flatPkgsByKey = keyBy(values(flatPkgs), key) // our critical set!
      const wpDepsByKey = keyBy(wpDeps, key)
      // mark all included deps as production
      for (let wpKey in wpDepsByKey) {
        if (flatPkgsByKey[wpKey]) flatPkgsByKey[wpKey].production = true
      }
      // remove production dependencies that were _not_ bundled
      const flatProdPkgsByKey = keyBy(values(flatPkgs).filter(p => p.production), key)
